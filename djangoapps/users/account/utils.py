...
def send_email_confirmation(request, user, signup=False):
    ...
    if email:
        ...
        if not user.verified:
            email_confirm = EmailConfirmation.objects \
                .filter(sent__gt=now() - COOLDOWN_PERIOD,
                        user=user)
            if not email_confirm.exists():
                email_confirm = EmailConfirmation.create(user=user)
                email_confirm.send_confirmation(request,
                                                signup=signup)
        else:
            send_email = False
