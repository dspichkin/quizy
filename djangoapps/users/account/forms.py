...
class SignupForm(BaseSignupForm):
    ...
    first_name = forms.CharField(label=_(u'Имя'), max_length=30, required=False)
    last_name = forms.CharField(label=_(u'Фамилия'), max_length=30, required=False)
    middle_name = forms.CharField(label=_(u'Отчество'), max_length=30, required=False)
...
class ResetPasswordForm(forms.Form):
    ...
    def clean_email(self):
        ...
        self.user = get_user_model().filter(email=email)
        if not self.user:
