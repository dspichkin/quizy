...
from users.account import app_settings as account_settings
from users.account.models import EmailConfirmation, EmailAddress
from users.socialaccount.models import SocialAccount, SocialToken
from users.socialaccount.providers import registry
from users.tests import MockedResponse
from users.account.signals import user_signed_up
from users.account.adapter import get_adapter
...
class GoogleTests(create_oauth2_tests(registry.by_id(GoogleProvider.id))):
    ...
    def test_google_compelete_login_401(self):
        ...
        with mock.patch('users.socialaccount.providers.google.views.requests') as patched_requests:
